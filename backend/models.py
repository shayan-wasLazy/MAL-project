...
class Anime(Base):
    ...
    tags = Column(String)
    main_characters = Column(String)
    main_staff = Column(String)

class UserAnime(Base):
    __tablename__ = 'user_anime'
    
    user_id = Column(Integer, ForeignKey('users.user_id'), primary_key=True)
    anime_id = Column(Integer, ForeignKey('anime_dataset.anime_id'), primary_key=True)
